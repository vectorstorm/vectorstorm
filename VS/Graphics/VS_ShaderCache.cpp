...
vsShaderCache::LoadShader( const vsString& vFile, const vsString& fFile, bool lit, bool texture )
{
	...
	if ( HasShader(uniqueName) )
	{
		...
		vsLog("Retrieved cached shader '%s', '%s', %d, %d: %f milliseconds", vFile, fFile, lit, texture, (after-before)/1000.f);
	...
	else
	{
		...
		unsigned long after = vsTimerSystem::Instance()->GetMicroseconds();
		vsLog("Loading shader '%s', '%s', %d, %d: %f milliseconds", vFile, fFile, lit, texture, (after-before)/1000.f);
